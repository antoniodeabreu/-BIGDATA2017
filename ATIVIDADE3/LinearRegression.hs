...
gradientDesc dataset α η = gradientDesc' w0 nab0
  where
    ...
    mse'    = mse dataset  --  troque para a versão paralela msePAR dataset
...
update dataset α η w nab = (w''.+. nab'', nab'')
  where
    nab'' = sumVecs nab'
    nab'  = calcNabla dataset e α -- troque para a versão paralela calcNablaPAR dataset e α
    e     = err dataset w -- troque para a versão paralela errPAR dataset w
    w'    = w  .+. (η*.nab)   -- momentum
...
calcNablaPAR :: [Point] -> [Double] -> Double -> [[Double]]
calcNablaPAR dataset e α = ((α/n) *. e) .*.. x `using`  parListChunk 300  rdeepseq
  where
    x = map fst dataset 	
    n = fromIntegral $ length dataset
...
errPAR :: [Point] -> [Double] -> [Double]
errPAR dataset w = (map snd dataset) .-. yhat `using`  parListChunk 100  rdeepseq
  where
    yhat = map fhat dataset 
    fhat point = sum $ w .*. (fst point)
...
msePAR :: [Point] -> [Double] -> Double
msePAR dataset w = mean $ concat $ result
  where
    result= map (.^ 2) chunks `using` parList rdeepseq
    e = err dataset w
    chunks =  chunksOf 200 e
...
meanPAR l = mean' 
  where
    mean'  = sum $ (map mean chunks `using` parList rdeepseq)
    chunks =  chunksOf 250 l
